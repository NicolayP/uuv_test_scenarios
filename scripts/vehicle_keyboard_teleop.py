...
class KeyBoardVehicleTeleop:
    ...
    CTRL-C to quit
        ...
        if rospy.has_param('~type'):
            ...
            if self._msg_type not in ['twist', 'accel', 'wrench']:
        ...
        elif self._msg_type == 'accel':
            self._output_pub = rospy.Publisher('output', Accel, queue_size=1)
        else:
            self._output_pub = rospy.Publisher('output', Wrench, queue_size=1)
    ...
    def _parse_keyboard(self):
        ...
        if self._msg_type == "twist" or self._msg_type == "accel":
            cmd.angular = self.a
            cmd.linear = self.l
        else:
            cmd.force = self.l
